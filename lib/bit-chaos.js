...
module.exports = class BitChaos {
  ...
  constructor (secret) {
    ...
    if(typeof secret === 'string') secret = TextAPI.encode(secret); else
    if(secret instanceof ArrayBuffer) secret = new Uint8Array(secret); else
    if(!(secret instanceof Uint8Array)) secret = Uint8Array.from(secret)
